...
import { g as getPersonaById } from './personas-CB5QIqw1.js';

function buildQuestionPrompt(persona, topic = "", recentPosts = []) {
  const topicLine = topic.trim() ? ` Keep the overall theme related to: "${topic}".` : "";
  let threadContext = "";
  if (Array.isArray(recentPosts) && recentPosts.length > 0) {
    const lines = recentPosts.slice(-12).map((p) => {
...
export { POST };
//# sourceMappingURL=_server-C_KSW1N_.js.map
